...
class CiscoTsmMR(TimesFmTorch):
  ...
  def forecast(self, 
    ...
    for start in range(0, N, batch_size):
      ...
      batch_coarse = torch.as_tensor(np.stack(coarse_contexts[start:end]),  dtype=torch.float32).unsqueeze(-1).to(self._device)
      batch_coarse_pad = torch.as_tensor(np.stack(coarse_pads[start:end]),  dtype=torch.float32).unsqueeze(-1).to(self._device)
      batch_fine = torch.as_tensor(np.stack(fine_contexts[start:end]), dtype=torch.float32).unsqueeze(-1).to(self._device)
      batch_fine_pad = torch.as_tensor(np.stack(fine_pads[start:end]), dtype=torch.float32).unsqueeze(-1).to(self._device)
